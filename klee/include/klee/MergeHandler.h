...
private:
  ...
  std::map<llvm::Instruction *, std::vector<ExecutionState *> >
      reachedCloseMerge;
  ///inline static unsigned int infer_exec_duration = 0;
  unsigned int infer_exec_duration = 0;

  /// @brief Unique identifier for the merge handler
...
public:
  ...
  /// X: merge or not
  bool merge_or_not(ExecutionState * es, nlohmann::json &json);
