...
StatsTracker::StatsTracker(Executor &_executor, std::string _objectFilename,
  ...
  klee_message("set up timer for statsWriteInterval");
  ...
  if (OutputStats) {
    ...
    klee_message("create table done. call writeStatsLine");
    writeStatsLine();

    if (statsWriteInterval){
      klee_message("Setting stats write interval to: %.2f seconds", statsWriteInterval.toSeconds());
      // std::cout << "Setting stats write interval to: " 
      //         << statsWriteInterval.toSeconds() << " seconds" << std::endl;
...
void StatsTracker::done() {
  if (statsFile){
    klee_message("stats tracker done, call writeStatsLine");
    writeStatsLine();
  }
...
void StatsTracker::stepInstruction(ExecutionState &es) {
  ...
  if (statsFile && StatsWriteAfterInstructions &&
    ...
    klee_message("after step into instruction.call writeStatsLine");
    writeStatsLine();
      }
...
void StatsTracker::writeStatsLine() {
  ...
  // printCallStack();
  klee_message("call writeStatsLine");
...
void StatsTracker::writeIStats() {
  ...
  // run command to output coverage information
  const char* outputfolder = globalOutputDirectory.c_str();
  klee_message("the output folder name is %s", outputfolder);
  std::string command = "klee-stats --print-all " + std::string(outputfolder);
  klee_message("the command to generate coverage information is %s", command.c_str());
  std::string output = exec(command.c_str());
  // Output the result to a file or handle it otherwise
  std::ofstream outFile(std::string(outputfolder) + "/coverage_output.txt", std::ios::app); // the folder is generated
  if (outFile.is_open()) {
    outFile << output;
    klee_message("New coverage information added to coverage_output.txt");
    outFile.close();
  } else {
