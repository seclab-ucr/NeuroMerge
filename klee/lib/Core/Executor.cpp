...
void Executor::branch(ExecutionState &state, 
  ...
  assert(N);
  // klee_message("Branching from original state at %p with %u conditions. Current Forks=%u, MaxForks=%u", &state, N, stats::forks, MaxForks.getValue());
...
Executor::fork(ExecutionState &current, ref<Expr> condition, bool isInternal) {
  ...
  // klee_message("fork is called at Br.");
  if (!isSeeding && !isa<ConstantExpr>(condition) && 
    ...
    if ((MaxStaticForkPct<1. &&
      ...
      Instruction *instr = current.pc->inst;
      // std::string instrString;
      // raw_string_ostream rso(instrString);
      // instr->print(rso);
      // klee_message("Fork condition at instruction %s exceeded MaxStaticForkPct limits.", 
      //              rso.str().c_str());
  ...
  if (res==Solver::True) {
    ...
    // compare openmergestack
    
    
    // std::string instrStr;
    // llvm::raw_string_ostream rso(instrStr);
    // trueState->pc->inst->print(rso);
    // KInstruction *ki = trueState->pc;
    // llvm::BasicBlock *block = ki->inst->getParent();
    // std::string function_name = block->getParent()->getName().str();
    // klee_message("forks happens at instruction: %s , function: %s , source location is: %s , the true state is: %p , the false state is: %p", rso.str().c_str(), function_name.c_str(), trueState->pc->getSourceLocation().c_str(), trueState, falseState);
    // // Logging MergeHandlers in the openMergeStack of trueState
    // klee_message("Listing MergeHandlers for trueState at address: %p", trueState);
    // for (const auto& handler : trueState->openMergeStack) {
    //     klee_message("  MergeHandler at address: %p", handler.get());
    // }

    // // Logging MergeHandlers in the openMergeStack of falseState
    // klee_message("Listing MergeHandlers for falseState at address: %p", falseState);
    // for (const auto& handler : falseState->openMergeStack) {
    //     klee_message("  MergeHandler at address: %p", handler.get());
    // }
  // unsigned numForkedStates = 0;
  // if (res == Solver::True || res == Solver::False) {
  //   klee_message("Forked 1 state due to definitive solver result.");
  //   numForkedStates = 1;
  // } else if (res == Solver::Unknown) {
  //   klee_message("Forked 2 states for unresolved condition.");
  //   numForkedStates = 2;  // This assumes that both states are viable and created
  // }

  // klee_message("Total new states forked after this call: %u", numForkedStates);
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  case Instruction::Br: {
    ...
    if (bi->isUnconditional()) {
      ...
      cond = optimizer.optimizeExpr(cond, false);
      Executor::StatePair branches = fork(state, cond, false); // trueState(state), falseState
...
void Executor::run(ExecutionState &initialState) { // select state and execute instruction
  bindModuleConstants();
  ...
  // After seeding or initial state insertion
klee_message("Initial total number of states: %zu", states.size());
for (auto state : states) {
    // Print some basic info about the state; you can customize this part
    klee_message("State at %p, PC at %p", state, state->pc);
}
  ...
  ExecutionState *lastState = nullptr; // Pointer to track the last state

  while (!states.empty() && !haltExecution) {
    // for (const auto& state : states) {
    //     std::cout << "State at address: " << &state << " has the following open merge handlers:\n";
    //     for (const auto& handler : state->openMergeStack) {
    //         std::cout << "  MergeHandler at address: " << handler.get() << std::endl;
    ...
    // Compare the current state pointer with the last state pointer
    if (lastState != &state) {
        // klee_message("Select state: %p", &state); // State has changed, dump the state
        // for (const auto& handler : state.openMergeStack) {
        //     std::cout << "  MergeHandler at address: " << handler.get() << std::endl;
        //     // Additional details can be printed here if needed
        // }
    ...
    KInstruction *ki = state.pc;
    // llvm::BasicBlock *block = ki->inst->getParent();
    // int instructionCount = std::distance(block->begin(), block->end());
    // klee_message("Current block at %p has %d instructions.", block, instructionCount);
    stepInstruction(state);

    // X: for auto merge
    // klee_message("Currently executing state at %p, PC at %p", &state, state.pc);
    forked = false;

    if (state.path.empty())
    {
      state.path.push_back(ki->inst);
    }
    if (state.path.back()->getParent() != ki->inst->getParent())
    {
      state.path.push_back(ki->inst);
    }
    executeInstruction(state, ki); // to do: insert timer here and print coverage information
    timers.invoke();
    if (::dumpStates) dumpStates(); // print log and collect information from log
    if (::dumpPTree) dumpPTree();
      ...
      if (i == bb->getTerminator() &&
          bb->getTerminator()->getNumSuccessors() > 1 && forked) {
          if (open_merge_map.find(bb) == open_merge_map.end()) { //open_merge_map: stores the map of start point to its end point. will return open_merge_map.end() if it doesn't find the end point for bb
              // klee_message("end point for bb %p not found, try to find it", bb);
              auto ff = bb->getParent();
              llvm::PostDominatorTree *PDT;
              if (this->function_to_PDT.find(ff) == this->function_to_PDT.end()) {
                  PDT = new llvm::PostDominatorTree();
                  PDT->recalculate(*ff);
                  this->function_to_PDT[ff] = PDT;
              } else {
                  PDT = this->function_to_PDT[ff];
              }

              llvm::BasicBlock *close_bb = bb->getTerminator()->getSuccessor(0);
              for (auto s : successors(bb)) {
                  close_bb = PDT->findNearestCommonDominator(close_bb, s);
                  if (close_bb == nullptr) {
                    break;
                  }
              }
              if (close_bb != nullptr) {
                  if (llvm::isPotentiallyReachable(bb, close_bb, nullptr, nullptr)) {
                      // klee_message("end point for bb %p is found at %p, call handler open merge", bb, close_bb);
                      open_merge_map[bb] = close_bb;// add the associted close point for bb in map
                      std::vector<ref<Expr>> arguments;
                      // std::cout << "Contents of close_merge_set before adding close_bb:" << std::endl;
                      // for (llvm::BasicBlock* bb : close_merge_set) {
                      //     // Print the pointer value (address)
                      //     std::cout << "BasicBlock at address: " << bb << std::endl;

                      //     // // If you want to print more detailed information, you might need to access
                      //     // // specific methods of llvm::BasicBlock. For example, to print the name:


                    //   if (const llvm::Instruction *Inst = bb->getFirstNonPHI()) {
                    //     if (const llvm::DebugLoc &Loc = Inst->getDebugLoc()) {  // Check if there is a debug location
                    //         unsigned Line = Loc.getLine();
                    //         llvm::StringRef File = Loc->getFilename();
                    //         klee_message("bb at %s:%u", File.str().c_str(), Line);
                    //     }
                    // }

                    //   if (const llvm::Instruction *closeInst = close_bb->getFirstNonPHI()) {
                    //       if (const llvm::DebugLoc &closeLoc = closeInst->getDebugLoc()) {  // Check for debug location in close_bb
                    //           unsigned closeLine = closeLoc.getLine();
                    //           llvm::StringRef closeFile = closeLoc->getFilename();
                    //           klee_message("close_bb at %s:%u", closeFile.str().c_str(), closeLine);
                    //       }
                    //   }
                      ...
                      // if (bb->hasName()) {
                      //   std::string function_name = bb->getParent()->getName().str();
                      //   std::string bb_name = bb->getName().str();
                      //   std::string bb_id = function_name + "::" + bb_name;
                      //   std::cout << "bb ID Name: " << bb_id << " " << state.pc->getSourceLocation().c_str() << std::endl;
                      // }
                      // if (close_bb->hasName()) {
                      //   std::string function_name = close_bb->getParent()->getName().str();
                      //   std::string close_bb_name = close_bb->getName().str();
                      //   std::string unique_id = function_name + "::" + close_bb_name;
                      //   std::cout << "close_bb ID Name: " << unique_id << " " << state.pc->getSourceLocation().c_str() << std::endl;
                      // }
                      // // }
                      // klee_message("[INFO] Opening merge at block: %p with state %p", bb, &state);
                      specialFunctionHandler->handleOpenMerge(state, ki, arguments);
                      close_merge_set.insert(close_bb);
                      // if (close_merge_map.find(bb) == close_merge_map.end()) {
                      //     // `close_bb` is not in the map, so add it with a counter of 1
                      //     close_merge_map[bb] = 1;
                      // } else {
                      //     // `close_bb` is already in the map, so increment the counter
                      //     close_merge_map[bb]++;
                      // }

                      // std::cout << "Contents of close_merge_set after adding close_bb:" << std::endl;
                      // for (llvm::BasicBlock* bb : close_merge_set) {
                      //     // Print the pointer value (address)
                      //     std::cout << "BasicBlock at address: " << bb << std::endl;
                      ...
                      //     // specific methods of llvm::BasicBlock. For example, to print the name:
                      //     if (bb->hasName()) {
                      //         std::cout << "  Name: " << bb->getName().str() << std::endl;
                      //     }
                      // }
                  } else {
                      llvm::errs() << "!llvm::isPotentiallyReachable(bb, close_bb, nullptr, nullptr)\n";
                  }
              } else {
                  llvm::errs() << "close_bb == nullptr\n";
              }
          } else {
              // klee_message("close point for bb %p is found in merge map, call handle open merge", bb);
              std::vector<ref<Expr>> arguments;
              // klee_message("[INFO] Opening merge at block: %p with state %p", bb, &state);
              // llvm::BasicBlock *close_bb_buffer = open_merge_map[bb];
              // std::string unique_id;
              // if (close_bb_buffer->hasName()) {
              //   std::string function_name = close_bb_buffer->getParent()->getName().str();
              //   std::string close_bb_name = close_bb_buffer->getName().str();
              //   std::string unique_id = function_name + "::" + close_bb_name;
              //   std::cout << "close_bb ID Name: " << unique_id << std::endl;
              // }
              specialFunctionHandler->handleOpenMerge(state, ki, arguments); // we can map the state with the merge point(close_bb ID)
          }
      }

    // X: for auto merge
    if (i == bb->getFirstNonPHI() &&
                close_merge_set.find(bb) != close_merge_set.end()) {// find the first instruction in the block that is not phi node and it's intended to be merged
      // klee_message("[INFO] Handling close merge...");
      // std::cout << "Contents of close_merge_set before calling handleCloseMerge:" << std::endl;
      //                 for (llvm::BasicBlock* bb : close_merge_set) {
      //                     // Print the pointer value (address)
      //                     std::cout << "BasicBlock at address: " << bb << std::endl;
      ...
      //                     }
      //                 }
      // auto it = close_merge_set.find(bb);
      // if (it != close_merge_set.end()) {
      //     // Found the BasicBlock in the set
      //     std::cout << "BasicBlock found in set at address: " << *it << std::endl;
      //     if ((*it)->hasName()) {
      //         std::cout << "  Name: " << (*it)->getName().str() << std::endl;
      //     }
      // } else {
      //     // BasicBlock not found in the set
      //     std::cout << "BasicBlock not found in set." << std::endl;
      // }
      std::vector<ref<Expr>> arguments;
      if (!state.openMergeStack.empty()){
      // klee_message("[INFO] Closing merge at block: %p with state %p", bb, &state);
      // if (bb->hasName()) {
      //   std::string function_name = bb->getParent()->getName().str();
      //   std::string bb_name = bb->getName().str();
      //   std::string unique_id = function_name + "::" + bb_name;
      //   std::cout << "bb ID Name at close point: " << unique_id << std::endl;
      // }
      state.inputfile = InputFile;
      specialFunctionHandler->handleCloseMerge(state, ki, arguments);
      // auto it = close_merge_map.find(bb);
      // int count = it->second;
      // // klee_message("[INFO] Closing merge at block: %p with state %p, count: %d", bb, &state, count);
      // for (int j = 0; j < count; ++j) {
      //       std::vector<ref<Expr>> arguments;  // Assuming arguments are prepared here if needed
      //       specialFunctionHandler->handleCloseMerge(state, ki, arguments);
      //       // Decrement the count in the map after each merge is handled
      //       it->second--;
      //   }
      // close_merge_map.erase(it);
      // std::vector<ref<Expr>> arguments;
      // specialFunctionHandler->handleCloseMerge(state, ki, arguments);
      // std::cout << "Contents of close_merge_set after calling handleCloseMerge:" << std::endl;
      //                 for (llvm::BasicBlock* bb : close_merge_set) {
      //                     // Print the pointer value (address)
      //                     std::cout << "BasicBlock at address: " << bb << std::endl;
  ...
  // to do: place to dump coverage
  // doDumpStates();
  // After exiting the loop

// klee_message("Execution completed. Remaining states: %zu", states.size());
// for (auto state : states) {
//     // Print final state information
//     klee_message("Final state at %p, PC at %p, PC's instruction at %p, Source location: %s", state, state->pc, (void*)(state->pc->inst), state->pc->getSourceLocation().c_str());
// }

  delete searcher; // recursively delete and call destructor. including states
  searcher = 0;
