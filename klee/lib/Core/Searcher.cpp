...
ExecutionState &BFSSearcher::selectState() {
  // klee_message("Select state: %p", *states.front());
  return *states.front();
}
...
ExecutionState &WeightedRandomSearcher::selectState() {
  double rngValue = theRNG.getDoubleL(); // Store the random value
  ExecutionState &selectedState = *states->choose(rngValue);
  // klee_message("Selected state with RNG value %f: %p", rngValue, &selectedState);
