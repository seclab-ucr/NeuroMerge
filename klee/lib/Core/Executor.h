...
struct KTest;
extern std::string globalOutputDirectory;
...
public:
  ...
  std::map<llvm::Instruction*, bool> mergeDecisionCache;
...
public:
  ...
  // X: use dominator tree for automatic merge
  std::map<llvm::Function *, llvm::PostDominatorTree *> function_to_PDT;
  std::map<llvm::BasicBlock *, llvm::BasicBlock *> open_merge_map;
  std::set<llvm::BasicBlock *> close_merge_set;
  // std::map<llvm::BasicBlock *, int> close_merge_map;
  bool forked;
};
