...
#include "../ToolLib/Logging.h"  // Adjust the path as necessary
...
//   std::string str;
//   raw_string_ostream dump(str);
//   sys::PrintStackTrace(dump);
//   return str;
// }
...
void SpecialFunctionHandler::handleOpenMerge(ExecutionState &state,
  ...
  // klee_message("before adding handler to openMergeStack of state %p", &state);
  //   std::cout << "Open Merge Stack contains " << state.openMergeStack.size() << " elements:" << std::endl;
    // for (const auto& handler : state.openMergeStack) {
    //     // Assuming MergeHandler has a method to get some identifiable information
    //     std::cout << "Handler at address: " << &*handler << std::endl;
    //     // If you can access more specific details, add those print statements here
    // }
  state.openMergeStack.push_back(
      ref<MergeHandler>(new MergeHandler(&executor, &state)));
  // klee_message("after adding handler to openMergeStack of state %p", &state);
  //   std::cout << "Open Merge Stack contains " << state.openMergeStack.size() << " elements:" << std::endl;
    // for (const auto& handler : state.openMergeStack) {
    //     // Assuming MergeHandler has a method to get some identifiable information
    //     std::cout << "Handler at address: " << &*handler << std::endl;
...
void SpecialFunctionHandler::handleCloseMerge(ExecutionState &state,
  ...
  if (state.openMergeStack.empty()) {
    ...
    // klee_message("empty openmergestack, Stack Trace: %s", stackTrace.c_str());// create print, this
    // printStackTrace();
    warning << &state << " ran into a close at " << i << " without a preceding open";
    klee_warning("%s", warning.str().c_str());
  } else {
    // klee_message("In handleCloseMerge");
    ...
    // klee_message("before pop handler from openMergeStack of state %p", &state);
    // std::cout << "Open Merge Stack contains " << state.openMergeStack.size() << " elements:" << std::endl;
    // for (const auto& handler : state.openMergeStack) {
    //     // Assuming MergeHandler has a method to get some identifiable information
    //     std::cout << "Handler at address: " << &*handler << std::endl;
    ...
    // klee_message("after pop handler from openMergeStack of state %p", &state);
    // std::cout << "Open Merge Stack contains " << state.openMergeStack.size() << " elements:" << std::endl;
