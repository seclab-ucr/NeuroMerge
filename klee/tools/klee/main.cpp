...
std::string globalOutputDirectory;
namespace {
  cl::opt<std::string>
...
public:
  ...
  std::string getOutputDirectory() const override {return m_outputDirectory.str();}
...
KleeHandler::KleeHandler(int argc, char **argv)
  ...
  if (dir_given) {
    ...
    // globalOutputDirectory = directory;
  } else {
    // "klee-out-<i>"
    int i = 0;
    for (; i <= INT_MAX; ++i) {
      ...
      if (mkdir(d.c_str(), 0775) == 0) {
        ...
        globalOutputDirectory = klee_last.c_str();
        klee_message("print symlink of output folder name %s", klee_last.c_str()); // check out the output folder name
        klee_message("print original output folder name %s", m_outputDirectory.c_str()); // check out the output folder name
...
std::string KleeHandler::getOutputFilename(const std::string &filename) {
  SmallString<128> path = m_outputDirectory;// looks like m_outputDirectory is the foldername
